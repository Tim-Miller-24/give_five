<template>
  <ModalsOverlay v-if="!isShowProductModal || isMobileOrTablet"
    :is-show="isShow"
    position="right"
    :offset="0"
    name="cart"
...
const showDeliveryPrice = ref(deliveryType.value?.includes('flat_rate') ? false : true);

onMounted(() => {
  if (selectedLocation?.value?.zone && deliveryType.value?.includes('flat_rate')) {
...
const { cartItems, cartItemsLength, cartItemsRegularPrice, cartItemsPrice, isShowDiscountBlock, discountBlock, discountProducts, promocode } = storeToRefs(cartStore)
