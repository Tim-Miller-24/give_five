        ...
        <form class="auth__form" @submit.prevent="onSubmit()">
            <transition name="fade" mode="out-in">
                <div v-if="step === 'phone'" class="auth__line">
                    ...
                    <span v-if="isNumberError" class="number-error">введите корректный номер</span>
                </div>
...
const isNumberError = ref<boolean>(false);
...
const SendCodeForTelegram = async () => {
  if (!phoneObj.value?.number?.length) {
  ...
  const data = await userStore.SendCodeForTelegram(obj, config);

  isLoading.value = false

  if (data?.value) {
    isNumberError.value = false;
    if (data.value.code_sended) {
      step.value = 'code';
    }
    else {
      step.value = 'code';
      const url = data.value.bot_url.startsWith('https://') ? data.value.bot_url : `https://${data.value.bot_url}`;
...
const checkCodeForTelegram = async () => {
  isLoading.value = true
